...
loop_transfer(iter_t iterations, void *cookie)
{
	...
	while (iterations-- > 0) {
		...
		while (nbytes > 0) {
			recv_ret = recv(sock, state->buf, state->msize, 0);
			if (recv_ret != state->msize) {
				/* 
				 * Could happen sometimes since recv() does not always receive 
				 * the data in the chunks that it was sent by send() 
				 */
...
server_main()
{
	char	*buf = (char*)valloc(MAX_BUFFER_SIZE);
